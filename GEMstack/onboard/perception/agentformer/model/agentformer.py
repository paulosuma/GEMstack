...
class ContextEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        for key in self.input_type:
            if key == "pos":
                ...
                hv = (
                    data["heading_vec"]
                    .unsqueeze(0)
                    .repeat((data["pre_motion"].shape[0], 1, 1))
                )
                traj_in.append(hv)
            elif key == "map":
                map_enc = (
        ...
        tf_in = self.input_fc(traj_in.view(-1, traj_in.shape[-1])).view(
            -1, 1, self.model_dim
        )
        agent_enc_shuffle = (
    ...
    def forward(self, data, reparam=True):
        ...
        if self.z_type == "gaussian":
            ...
            data["q_z_dist"] = Categorical(
                logits=q_z_params, temp=self.z_tau_annealer.val()
            )
...
class FutureDecoder(nn.Module):
        ...
        for i in range(self.future_frames):
            ...
            if self.pred_type == "scene_norm" and self.sn_out_type in {"vel", "norm"}:
                norm_motion = seq_out.view(
                    -1, agent_num * sample_num, seq_out.shape[-1]
                )
    ...
    def forward(
        self, data, mode, sample_num=1, autoregress=True, z=None, need_weights=False
    ):
...
class AgentFormer(nn.Module):
    ...
    def set_data(self, data):
        ...
        if self.training and len(data["pre_motion_3D"]) > self.max_train_agent:
            ...
            ind = np.random.choice(
                len(data["pre_motion_3D"]), self.max_train_agent
            ).tolist()
            for key in [
                "pre_motion_3D",
                "fut_motion_3D",
        ...
        self.data["pre_motion"] = (
            torch.stack(in_data["pre_motion_3D"], dim=0)
            .to(device)
            .transpose(0, 1)
            .contiguous()
        )
        self.data["fut_motion"] = (
            torch.stack(in_data["fut_motion_3D"], dim=0)
            .to(device)
            .transpose(0, 1)
            .contiguous()
        )
        self.data["fut_motion_orig"] = torch.stack(in_data["fut_motion_3D"], dim=0).to(
            device
        )  # future motion without transpose
        self.data["fut_mask"] = torch.stack(in_data["fut_motion_mask"], dim=0).to(
            device
        )
        self.data["pre_mask"] = torch.stack(in_data["pre_motion_mask"], dim=0).to(
            device
        )
        ...
        if self.rand_rot_scene and self.training:
            ...
            for key in ["pre_motion", "fut_motion", "fut_motion_orig"]:
                self.data[f"{key}"], self.data[f"{key}_scene_norm"] = rotation_2d_torch(
                    self.data[key], theta, self.data["scene_orig"]
                )
            if in_data["heading"] is not None:
        ...
        self.data["pre_vel"] = (
            self.data["pre_motion"][1:] - self.data["pre_motion"][:-1, :]
        )
        ...
        self.data["pre_motion_norm"] = (
            self.data["pre_motion"][:-1] - self.data["cur_motion"]
        ...
        if self.use_map:
            ...
            scene_points = (
                np.stack(in_data["pre_motion_3D"])[:, -1] * data["traj_scale"]
            )
            ...
            self.data["agent_maps"] = scene_map.get_cropped_maps(
                scene_points, patch_size, rot
            ).to(device)

        # agent shuffling
        if self.training and self.ctx["agent_enc_shuffle"]:
            self.data["agent_enc_shuffle"] = torch.randperm(self.ctx["max_agent_len"])[
                : self.data["agent_num"]
            ].to(device)
        else:
    ...
    def compute_loss(self):
        ...
        for loss_name in self.loss_names:
            loss, loss_unweighted = loss_func[loss_name](
                self.data, self.loss_cfg[loss_name]
            )
            total_loss += loss
