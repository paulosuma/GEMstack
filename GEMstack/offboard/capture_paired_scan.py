...
sensor_env_path = '../knowledge/vehicle/gem_e4_sensor_environment.yaml'
with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), sensor_env_path), 'r') as f:
    sensor_env = yaml.safe_load(f)
...
def main(folder='data', start_index=1):
    rospy.init_node('capture_paired_scan', disable_signals=True)
    lidar_sub = rospy.Subscriber(sensor_env['ros_topics']['top_lidar'], PointCloud2, lidar_callback)
    camera_sub = rospy.Subscriber(sensor_env['ros_topics']['front_camera'], Image, camera_callback)
    depth_sub = rospy.Subscriber(sensor_env['ros_topics']['front_depth'], Image, depth_callback)
    index = start_index
